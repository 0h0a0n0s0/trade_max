...
class WorkflowManager:
    ...
    async def _run_daily_validation(self):
        ...
        try:
            # 使用當前配置執行回測
            # Note: Import path updated after refactoring
            import sys
            import os
            ...
            # [Fix] Use strictly the last 24 hours for validation
            last_ts = price_df.index[-1]
            one_day_ago = last_ts - pd.Timedelta(hours=24)
            validation_df = price_df[price_df.index >= one_day_ago].copy()
            
            if len(validation_df) < 60:  # Ensure at least 1 hour of data
                LOG.warning("Not enough data for daily validation (less than 60 candles).")
                return
            ...
            LOG.info(f"Daily validation (24h) completed. ROI: {validation_roi:.4f}")
