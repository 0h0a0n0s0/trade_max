...
Recommended execution command (4-core optimization):
    python core/optimizer.py \
        ...
        --output-csv data/optimization_results.csv
"""
import argparse
import json
import re
...
def objective(
    ...
    trend_ema_slow_bars = trial.suggest_int("trend_ema_slow_bars", 4320, 14400, step=1440)   # 3d ~ 10d
    trend_ema_fast_bars = trial.suggest_int("trend_ema_fast_bars", 60, 480, step=30)         # 1h ~ 8h
    # Simple hard-stop drawdown protection
    max_drawdown_stop_pct = trial.suggest_float("max_drawdown_stop_pct", 0.15, 0.30)         # 15% ~ 30%
    # Force pure_grid mode for high-frequency trading
    if strategy_mode == 'pure_grid' or True:  # Always use aggressive grid mode
        # Grid gap: 0.5% ~ 2.0% of price
        low_gap = min_gap * 5.0              # 0.5%
        high_gap = max_gap                   # 2.0%
        small_gap = trial.suggest_float("small_gap", low_gap, high_gap)
        ...
        # Unlock Position Bias: Neutral zone still crypto-heavy (0%~50% USDT)
        bias_neutral_target = trial.suggest_float("bias_neutral_target", 0.0, 0.5)
